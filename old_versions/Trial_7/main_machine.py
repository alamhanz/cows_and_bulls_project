...
def data_val(pri,pos,co):
	...
	if len(dv)>25 and st>0.0:
		dv2=[]
		z=0.1
		while len(dv2)<3 and z<2.5:
			dv2=dv[dv.pri_prob<=mp+(z*st)].element.values
			z+=0.3
	elif st2>0.0:
		dv2=[]	
		z=0.1
		while len(dv2)<3 and z<2.5:
			dv2=dv[dv.pri_prob>=mp2-(z*st)].element.values
			z+=0.3	
		# # print co
		# print dv
		# print mp2
...
class prior_prob():
	...
	def update(self,answer,old_guess):
		...
		self.former_guess.append(old_guess)

		for ans in self.element:
			if ans not in old_guess:
				if self.all_prior[ans]==-1:
					self.all_prior[ans]=(1-prob_take)*(1-(sum(answer)/4.0))
				else:
					self.all_prior[ans]=(self.all_prior[ans]*(1-(sum(answer)/4.0))*(1-prob_take))
			else:
				if self.all_prior[ans]==-1:
					self.all_prior[ans]=prob_take*((sum(answer)/4.0))
				else:
					self.all_prior[ans]=(self.all_prior[ans]*((sum(answer)/4.0))*(prob_take))
