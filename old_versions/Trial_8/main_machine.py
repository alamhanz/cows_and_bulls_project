...
def data_val(pri,pos,co):
	...
	if len(dv)>50 and st>0.0:
		...
		z=0.1
		while len(dv2)<3 and z<1.8:
			dv2=dv[dv.pos_prob>=mp2-(z*st)].element.values
			z+=0.3	
		# print co
		# print dv
		# print mp2
		...
		for ans in self.element:
			if ans not in old_guess:
				if self.all_prior[ans]==-1:
					self.all_prior[ans]=((4-sum(answer))/10.0)+((2+sum(answer))/10.0)
				else:
					self.all_prior[ans]=self.all_prior[ans]*(((4-sum(answer))/10.0)+((2+sum(answer))/10.0))
			else:
				if self.all_prior[ans]==-1:
					self.all_prior[ans]=((sum(answer)/10.0))+((4-sum(answer))/10.0)
				else:
					self.all_prior[ans]=self.all_prior[ans]*(((sum(answer)/10.0))+((4-sum(answer))/10.0))
